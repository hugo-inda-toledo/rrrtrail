                ...
                <div class="col-lg-2" id="session-input-id">
                    <div class="form-group" id="session-input-div">
    	            	<?php if(isset($stores_list)):?>
                            ...
                            <?php echo $this->Form->select('session_date', $sessions_list, ['class' => 'form-control', 'type' => 'select', 'label' => false, 'required' => 'required', 'empty' => __('Select a session'), 'id' => 'session_date']);?>
    	            	<?php endif;?>
    	            </div>
                </div>
...
<?php $this->Html->scriptStart(array('block' => 'scriptBottom', 'inline' => false)); ?>
    ...
    $(document).ready(function(){
        ...
        $('#generate-button').on('click', function(){
        	...
        	if(pass == true){
        		...
        		$.ajax({
                    url: webroot + 'ajax/robotReports/doStockOutReport/',
                    data: {
    ...
    function getRobotSessions(){
    	...
    	if(store_id != ''){
    		...
    		$.ajax({
                url: webroot + 'robot-sessions/getSessionsList/'+ store_id + '/facing/true/true',
                cache: false,
                type: 'POST',
                dataType: 'json',
                success: function (response) 
    ...
    function getSessionsFromDate(store_id, date_string){
        ...
        if(store_id != '' && date_string != ''){
            ...
            $.ajax({
                url: webroot + 'robot-sessions/getSessionsByDate/'+ store_id + '/facing/' + date_string,
                cache: false,
                type: 'POST',
                dataType: 'json',
                success: function (response) 
