 ...
 *
 *
 * @method \Suppliers\Model\Entity\Store[]|\Cake\Datasource\ResultSetInterface paginate($object = null, array $settings = [])
 */
    ...
    public function details($store_code = null){
        if($store_code != null){
            ...
            if(count($stores_list) == 0){
                $this->Flash->warning(__("You don't access to the {0} store", $store_code));
                $this->redirect($this->referer());
            }
            ...
            $last_robot_session = $this->Stores->RobotSessions->find('all')
                ->where([
            ...
            $data = [];
            $this->set('data', $data);
            $this->set('robot_session_data', $last_robot_session);
